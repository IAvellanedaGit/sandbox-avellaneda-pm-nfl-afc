...
import (
	...
	"github.com/IAvellanedaGit/sandbox-avellaneda-pm-nfl-afc/service/model"
...
func NewManager(b *bridge.Bridge) *Manager {
	...
	m.bus.Subscribe("nfl:afc:initialize", m.initialize)
...
func (mgr *Manager) initialize(m *bridge.Message) ([]byte, error) {
	...
	mgr.logger.Debug("Initializing NFL AFC Manager [%s] LogLevel [%d]",
		AFC_VERSION,
		request.LogLevel())

	mgr.logger = bridge.NewLogger("afc", bridge.LogLevel(request.LogLevel()), mgr.logger)

	mgr.svc = NewService(mgr.b, mgr.logger)

	// subscribe to other messages
	mgr.bus.Subscribe("nfl:afc:list-teams", mgr.listTeams)
	mgr.bus.Subscribe("nfl:afc:validate-team", mgr.validateTeam)
	mgr.bus.Subscribe("nfl:afc:predict-score", mgr.predictScore)

	builder := flatbuffers.NewBuilder(0)

	vOffset := builder.CreateString(AFC_VERSION)
